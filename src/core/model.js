...
export const getProfile = async () => {
  const avatar =
    (await AsyncStorage.getItem('desocial@0313/profilePhoto')) || 'anonymous'
  ...
  const instagram =
    (await AsyncStorage.getItem('desocial@0313/userInstagram')) || ''
  const linkedin =
    (await AsyncStorage.getItem('desocial@0313/userLinkedin')) || ''
  ...
  const followingStatus = await AsyncStorage.getItem(
    'desocial@0313/followingStatu'
  )
...
export const setProfile = async ({
  ...
  if (avatar != undefined)
    await AsyncStorage.setItem('desocial@0313/profilePhoto', avatar)
